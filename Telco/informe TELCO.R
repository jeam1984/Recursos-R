...
```{r calculo de datos, include=FALSE}

glimpse(df)

num_datos<- nrow(glimpse(df))
num_cols<- ncol(glimpse(df))
...
* Big Data Analytics aplicado al negocio: 
  Análisis tasa de abandono empresa de telecomunicaciones
...
1. Podemos observar que el archivo contiene: `r format(num_datos,digits = 0,decimal.mark = ',',big.mark = '.')` registros y  `r format(num_cols,digits = 0,decimal.mark = ',',big.mark = '.')` variables, dentro de las cuales destacan los principales servicios prestados por la compañía, como teléfono, internet, televisión...etc. así como los datos más propios del cliente, como género, forma de pago, importe facturas, etc. y por último la **tasa de abandono**, considerada target y sobre la cual desarrollamos el modelo predictivo.
...
1. **Importación**: Cargamos los datos 
Usamos fread de data.table para una lectura mucho mas rapida
```{r}
df <- fread('Telco.csv')
```

2.  **Muestreo**: Analisis exploratorio
```{r Exploración general}
glimpse(df)
```

2.1. Calidad de datos: Estadísticos básicos
...
Nota: Vamos a probar dos algoritmos diferentes para ver cual funciona mejor y aprender como se comparan
...
Ademas vamos a incluir un nuevo paramtero para que el arbol no tenga mas de 7 niveles
```{r}
